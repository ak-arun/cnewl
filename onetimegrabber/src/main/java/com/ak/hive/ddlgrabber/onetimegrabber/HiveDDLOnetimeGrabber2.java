...
import com.ak.hive.ddlgrabber.onetimegrabber.db.DDLPersistTask2;
...
public class HiveDDLOnetimeGrabber2 {
	
	private static final Logger LOG = LoggerFactory.getLogger(HiveDDLOnetimeGrabber2.class);
	...
	public static void main(String[] args) throws DBException, FileNotFoundException, IOException, SQLException, InterruptedException, KeyStoreException, NoSuchAlgorithmException, CertificateException, UnrecoverableKeyException {
		...
		for(List<DDLObject> ddlObjects : ddlPartitions){
			executor.execute(new DDLPersistTask2(ddlObjects, confHive, confDestDb, properties.getProperty("ddlstore.tablename"),latch));
		}
		latch.await();
		executor.shutdown();
